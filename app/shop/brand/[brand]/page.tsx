...
export default function ShopBrandPage() {
  ...
  }, [searchTerm, minPrice, maxPrice, sortBy, currentPage, brand]);

  const breadcrumbs = (
    <nav className="text-sm mb-6 text-gray-600">
      <Link href="/" className="hover:underline">Home</Link> &gt; <Link href="/products" className="hover:underline">Shop</Link> &gt; <span>{brand}</span>
    </nav>
  );
